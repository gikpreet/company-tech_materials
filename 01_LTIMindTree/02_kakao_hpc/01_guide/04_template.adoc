...
+
----
[azureuser@dm-cyclecloud ~]$ git clone https://github.com/Azure/cyclecloud-pbspro.git
...
----
+
4. 편집기를 사용해서 템플릿의 이름을 _OpenPBS_HPC_ 로 변경합니다. 이 이름은 CycleCloud 관리 포털에서 템플릿 목록을 보거나 CLI에서 템플릿을 지정할때의 이름입니다.
+
----
################################
...
1. 열려 있는 openpbx_hpc.txt 파일에서, **[cluster OpenPBS_HPC]** 섹션에서 **[[nodearray execute]] 섹션** 아래에 다음을 추가합니다.
+
----
...
+
----
[azureuser@dm-cyclecloud cyclecloud-pbspro]$ cat project.ini | grep version
version = 2.0.25
----
+
...
+
----
[azureuser@dm-cyclecloud blobs]$ pwd
...
+
----
[azureuser@dm-cyclecloud blob]$ ls
...
+
----
[azureuser@dm-cyclecloud templates]$ cyclecloud import_template -f openpbs_hpc.txt
...
+
----
cyclecloud delete_template OpenPBS
----
+
3. CycleCloud 포털에서, 생성한 OpenPBS 템플릿이 제거된 것을 확인합니다.
+
image:./images/04/04/image03.png[width=700]